...
case class PostgresDoc(id: Key, data: JsValue)

object PostgresDoc {
  implicit val format = Json.format[PostgresDoc]
}

object PostgresJsonDataStore {
  def apply[F[_]: Effect](mongoApi: ReactiveMongoApi,
                          config: DbDomainConfig)(implicit actorSystem: ActorSystem): PostgresJsonDataStore[F] =
    new PostgresJsonDataStore[F](config.conf.namespace, mongoApi)
}

class PostgresJsonDataStore[F[_]: Effect](namespace: String, mongoApi: ReactiveMongoApi)(
  ...
  override def create(id: Key, data: JsValue): F[Result[JsValue]] = {
    ...
    storeCollection
      .map(_.insert(PostgresDoc(id, data)))
      .map { _ =>
        Result.ok(data)
      }
      .toF
  ...
  private def updateRaw(id: Key, data: JsValue)(implicit collection: JSONCollection): F[Result[JsValue]] =
    collection.update(Json.obj("id" -> id.key), PostgresDoc(id, data), upsert = true).toF.map(_ => Result.ok(data))

  private def createRaw(id: Key, data: JsValue)(implicit collection: JSONCollection): F[Result[JsValue]] =
    collection.insert(PostgresDoc(id, data)).toF.map(_ => Result.ok(data))
  ...
  override def getByIdLike(patterns: Seq[String], page: Int, nbElementPerPage: Int): F[PagingResult[JsValue]] = {
    ...
    storeCollection.toF.flatMap { implicit collection =>
      val findResult: F[Seq[PostgresDoc]] = collection
        .find(query)
        .options(options)
        .cursor[PostgresDoc](ReadPreference.primary)
        .collect[Seq](maxDocs = nbElementPerPage, Cursor.FailOnError[Seq[PostgresDoc]]())
        .toF
